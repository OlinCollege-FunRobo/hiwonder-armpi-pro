...
def main():
    ...
    try:
        while True:
            ...
            print("1: Read all joint positions")
            print("2: Set motor speeds")
            ...
            if case == 0:
                try:
                    joint_id = int(input("Enter joint ID (0-5): ").strip())
            ...
            if case == 1:
                try:
                    #joint_id = int(input("Enter joint ID (0-5): ").strip())
                    #print(f"Position of joint {joint_id}: {robot.joint_values[joint_id]}")
                    print(f"Joint positions: {robot.get_joint_values()}")
                except ValueError:
                    print("Unable to get joint values")
